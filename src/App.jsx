...
function AdminView({ context, sessions, downloads, events, facilitators, onContentSaved, onOrganizationSaved, onArchived }) {
  ...
  return <CollectionView title="Administración" eyebrow="ESPACIO DE OPERACIÓN" description="Gestiona el contenido de tu organización desde un solo lugar." icon={Users}>
    ...
    <div className="admin-workspace"><OrganizationSettings organization={organization} onSaved={onOrganizationSaved} /><ContentCreator organization={organization} canManageEvents={context?.permissions?.can_manage_events} onSaved={onContentSaved} /><AdminContentList title="Sesiones" type="session" items={sessions} onArchived={onArchived} /><AdminContentList title="Eventos" type="event" items={events} onArchived={onArchived} /><AdminContentList title="Descargables" type="download" items={downloads} onArchived={onArchived} /><FacilitatorList facilitators={facilitators} onArchived={onArchived} /></div>
  </CollectionView>;
}
