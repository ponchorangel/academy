...
export default function App() {
  ...
  async function resendInvitation(invitation) {
    const result = await base44.functions.invoke("academyInvitationMutation", { action: "resend", invitation_id: invitation?.id });
    const payload = result?.data || result;
    if (payload?.invitation) {
      setInvitations((current) => current.map((item) => item.id === payload.invitation.id ? payload.invitation : item));
      recordAudit("resend.invitation", "invitation", payload.invitation.id);
    }
  }
...
function AdminView({ context, sessions, downloads, events, courses, courseModules, courseLessons, facilitators, members, invitations, onContentSaved, onOrganizationSaved, onArchived, onMemberChanged, onInvitationCreated, onResendInvitation, onTenantCreated, onNavigate }) {
  ...
  return <CollectionView title="Administración" eyebrow="ESPACIO DE OPERACIÓN" description="Gestiona el contenido de tu organización desde un solo lugar." icon={Users}>
    ...
    <LaunchCenter onboarding={context?.onboarding} onNavigate={onNavigate} />
    ...
    <div className="admin-workspace">{context?.permissions?.can_manage_organization && <OrganizationSettings organization={organization} onSaved={onOrganizationSaved} />}<ContentCreator organization={organization} canManageEvents={context?.permissions?.can_manage_events} facilitators={facilitators} editingItem={editing?.item} editingType={editing?.type} onClearEdit={() => setEditing(null)} onSaved={(type, item, mode) => { onContentSaved(type, item, mode); setEditing(null); }} /><CourseBuilder organization={organization} courses={courses} modules={courseModules} lessons={courseLessons} onSaved={onContentSaved} onArchived={onArchived} />{context?.permissions?.can_manage_organization && <MemberManager organization={organization} members={members} invitations={invitations} canManageOrganization={context?.permissions?.can_manage_organization} onMemberChanged={onMemberChanged} onInvitationCreated={onInvitationCreated} onResendInvitation={onResendInvitation} />}<AdminContentList title="Sesiones" type="session" items={sessions} onArchived={onArchived} onEdit={(type, item) => setEditing({ type, item })} /><AdminContentList title="Cursos" type="course" items={courses} onArchived={onArchived} onEdit={(type, item) => setEditing({ type, item })} /><AdminContentList title="Eventos" type="event" items={events} onArchived={onArchived} onEdit={(type, item) => setEditing({ type, item })} /><AdminContentList title="Descargables" type="download" items={downloads} onArchived={onArchived} onEdit={(type, item) => setEditing({ type, item })} /><FacilitatorList facilitators={facilitators} onArchived={onArchived} onEdit={(type, item) => setEditing({ type, item })} /></div>
  </CollectionView>;
}

function LaunchCenter({ onboarding, onNavigate }) {
  if (!onboarding) return null;
  const labels = { branding: "Personalizar marca", welcome: "Escribir bienvenida", modules: "Elegir módulos", responsible: "Invitar administrador", facilitator: "Crear facilitador", first_resource: "Publicar primer recurso", first_student: "Invitar primer alumno", custom_domain: "Conectar dominio" };
  const targets = { branding: "academy-branding", welcome: "academy-branding", modules: "academy-modules", responsible: "academy-members", facilitator: "academy-content", first_resource: "academy-content", first_student: "academy-members", custom_domain: "academy-branding" };
  function go(key) { onNavigate?.("administracion"); window.setTimeout(() => (document.getElementById(targets[key]) || document.querySelector(".admin-workspace"))?.scrollIntoView({ behavior: "smooth", block: "start" }), 50); }
  return <section className="admin-panel launch-center"><div className="admin-panel-heading"><div><p className="eyebrow">CENTRO DE LANZAMIENTO</p><h3>{onboarding.percentage === 100 ? "Academy lista para operar" : "De cero a tu primer taller"}</h3></div><span className="onboarding-progress">{onboarding.completed}/{onboarding.total} · {onboarding.percentage}%</span></div><p className="panel-description">Completa estos pasos para entregar una Academy lista para recibir alumnos e impartir su primer recurso.</p><div className="launch-track"><span style={{ width: `${onboarding.percentage}%` }} /></div><div className="launch-steps">{Object.entries(labels).map(([key, label]) => <div className={`launch-step ${onboarding.items[key] ? "done" : ""}`} key={key}><span className="onboarding-check">{onboarding.items[key] ? "✓" : ""}</span><div><strong>{label}</strong><small>{onboarding.items[key] ? "Completado" : "Pendiente"}</small></div>{!onboarding.items[key] && <button type="button" className="text-link" onClick={() => go(key)}>Abrir</button>}</div>)}</div>{onboarding.percentage === 100 && <div className="launch-ready"><strong>Lista para el piloto</strong><span>Ya puedes compartir el acceso e impartir sesiones, cursos y webinars.</span></div>}</section>;
}
...
function OnboardingChecklist({ onboarding }) {
  if (!onboarding) return null;
  const labels = { branding: "Subir el logo", welcome: "Escribir bienvenida", modules: "Elegir módulos", responsible: "Asignar responsable", facilitator: "Crear facilitador", first_resource: "Publicar primer recurso", first_student: "Invitar primer alumno", custom_domain: "Conectar dominio" };
...
function MemberManager({ organization, members, invitations, canManageOrganization, onMemberChanged, onInvitationCreated, onResendInvitation }) {
